...
def test_prediction(popt,variables,z,poly,zerr_):
    """
...
er_results_p=test_prediction(popt,variables_p,z_p,poly_p,zerr_p)
...
x,y=np.meshgrid(np.linspace(np.min(x_p),1.4,20),np.linspace(np.min(y_p),0.5,20))



z=np.asarray(x)
variables_p=np.stack((x.flatten(),y.flatten()),axis=0)
er_results=test_prediction(popt,variables_p,z.flatten(),poly_p,zerr_p)
...
er_results_e=test_prediction(popt,variables_e,z_e,poly_e,zerr_e)
#    
...
#    
#Creating the surface
x,y=np.meshgrid(np.linspace(np.min(x_e),1.4,20),np.linspace(np.min(y_e),0.5,20))



z=np.asarray(x)
variables_e=np.stack((x.flatten(),y.flatten()),axis=0)
er_results=test_prediction(popt,variables_e,z.flatten(),poly_e,zerr_e)
z_mesh=np.reshape(er_results[:,1],np.shape(x))
