...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 15 17:40:14 2020
COPY of gk transport just to test different sampling options in 6.GK_analysis_2
@author: simon
"""
...
for i, folder in enumerate(folders):
    ...
    for j,sim in  enumerate(sim_array):
        if i == 0 and j == 0:            
        ...
        # The name of the 
        logger.info("Analysing the file %s"%sim.flux_file)
        # =============================================================================
...
for i,corr in enumerate(correlation_dist):
    m_qs_array = []
    for t in tau_array:
        # Computing the value until t
        m_qs_inst = prefactor * cf.integrate(m_qs.times, corr, 0, t,)   
        m_qs_array.append(m_qs_inst)


    m_qs_array = np.array(m_qs_array)
    
    ax.plot(tau_array,m_qs_array, label =sim_array[i].name)
