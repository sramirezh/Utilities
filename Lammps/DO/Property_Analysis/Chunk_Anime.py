...
It generates a file "Averages.dat" that has the averages of all data and then 
if there are stress per atom, computes the 
pressure.

The positions of the stresses have to be given explicitly as CSV, and the index
 counting starts at zero in chunk, for example:
    ['Chunk', 'Coord1', 'Ncount', 'vx', 'vz', 'density/mass', 'c_Stress[3]'], 
    c_Stress[3] is the 6th entry. 
"""
...
if len(InputArg)==1:
    pressFlag=0
    print("\nNo stress calculations explicitly required, searching for the word
     'stress' in any of the Parameters...\n")
else:
    pressFlag=1
...
#Excluded parameters, like the position of the chunk
ExcludeP = 2 
Parameters.remove("#")
...
for k in range(x): 
   # print("Reading configuration %d of %d" %(k,x-1))
    ...
    plt.pause(0.01)
    plt.clf()
    #Runs over the parameter
    for l in range(ExcludeP, Nparam): 
        if IsPress[l] == 1:
            #Stress per atom*mass/density
            Averages[:, l] = Averages[:, l] - Chunk_Results[:, l] * 
            Chunk_Results[:, index] 
        else:
            Averages[:, l] = Averages[:, l] + Chunk_Results[:, l]
    #To set the Pyy
    #Averages[:,1]=Averages[:,1]+Chunk_Results[:,5]*Chunk_Results[:,6]      
