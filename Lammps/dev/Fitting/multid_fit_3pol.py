...
A really messy algorithm to fit 3 data sets simultaneously
@author: simon
"""
import numpy as np
import argparse
...
def three_poly(data,*params):
    ...
    data_1=data[0][:,:len(x_p)]  #Correct because x_e is global
    poly_1=poly_p
    data_2=data[0][:,len(x_p):len(x_p)+len(x_e)]
    poly_2=poly_e
    data_3=data[0][:,len(x_p):len(x_p)+len(x_e):]
    poly_3=poly_a
...
def main(rho_ref,beta_ref,a_ref,deg_x,deg_y,p_ref,e_ref,file_p,file_e,file_a):
    
    global popt,poly_p,poly_e,poly_a,x_p,x_e,x_a
...
# =============================================================================
    x_a,y_a,z_a,zerr_a=read_data(file_a,rho_ref,beta_ref,a_ref)
    z_a=z_a*x_a*y_a
...
# =============================================================================
    ...
    z_predict_a,error_a=test_prediction(popt,variables_a,z_a,poly_a)
#    
    outputs(popt,pcov,error_a, deg_x, deg_y,'a')
#    
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(x_a, y_a, z_a, zdir='z',marker='.',label="Simulation",color='r')
#    ax.scatter(x, y, z_predict, zdir='z',label="Fitting",color='black')
    ...
    z_mesh,error=test_prediction(popt,variables_a,z.flatten(),poly_a)
    z_mesh=np.reshape(z_mesh,np.shape(x))
    ax.plot_wireframe(x,y,z_mesh,color='b')
    ax.set_xlabel(r'$\rho-\rho^*$')
    ax.set_ylabel(r'$\beta-\beta^*$')
    ax.set_zlabel(r'$\Delta \rho \beta \ a_{exc}$')
    
    fig.legend()
    fig.savefig("3Dplot_a.pdf")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-file_a', metavar='file_a',help='Input filename with energy results',default='LA')
    parser.add_argument('-beta_ref', metavar='beta reference',help='reference beta',default=1,type=float)
    parser.add_argument('-rho_ref',metavar='rho reference',help='reference rho',default=0,type=float )
    parser.add_argument('-a_ref',metavar='Free energy reference',help='reference rho',default=0,type=float )
    ...
    main(args.rho_ref,args.beta_ref,args.a_ref,args.deg_x,args.deg_y,args.P_ref,args.E_ref,args.file_p,args.file_e,args.file_a)
